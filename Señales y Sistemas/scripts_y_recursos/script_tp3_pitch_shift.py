...
def procesar_y_graficar(fragmento, nota):
    ...
    Tf_senal_original = np.fft.fft(data_notas, norm="forward")
    L_primer_nota = len(data_notas)
    ...
    Tf_senal_shifteada = np.fft.fft(datos_shifteados, norm="ortho")
    L_primer_nota = len(datos_shifteados)
