...
plt.pcolormesh(t_spec, f, 10 * np.log10(Sxx), shading='gouraud', vmin = -100, vmax = -20)
plt.title("Espectograma de la señal - Componentes armónicos")
plt.ylim(0, 5000)
plt.ylabel('Frecuencia [Hz]')
plt.xlabel('Tiempo [s]')
plt.colorbar(label='Intensidad [dB]')
...
plt.pcolormesh(t_spec, f, 10 * np.log10(Sxx), shading='gouraud', vmin = -120, vmax = -40)
plt.title("Espectograma de la señal - Componentes percusivos")
plt.ylim(0, 5000)
plt.ylabel('Frecuencia [Hz]')
plt.xlabel('Tiempo [s]')
plt.colorbar(label='Intensidad [dB]')
